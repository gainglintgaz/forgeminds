...
- [x] **E0 — deploy-readiness DONE.** `master` reattached, fully pushed to `origin/master` (10 commits landed 2026-07-09: E2, H1 doc, tracking, I1 doc, H1 build ×5). A throwaway Vercel smoke-deploy (private preview URL, per the plan's Hosting Timeline exception) confirms the app boots on the target host — NOT the E6 host cutover, NOT publishing, no env vars wired yet.
- [x] **E1 — finance-first brief BUILT + PROVEN LOCALLY** (commit `1123f8b`). Antarctica/relevance gate: **PASS**, zero generic-news leak.
- [x] **E2 — tickers + market data BUILT + PROVEN LOCALLY** (commit `a8f31d2`). `n_tickers` 1→11; real prices woven in.
- [x] **🧑 FOUNDER VERDICT ON E1/E2 (2026-07-09): FAIL vs Pipedream** — the relevance fix worked, but the *package* lost: no charts, no social drafts, no video prompts, no source links, and the rendered market data was an unrelated full-watchlist dump instead of story-tied data, plus the brief read as "surface-level, everyone-knows" news with no causal "why." Full benchmark + corrections captured in **`docs/ops/PIPEDREAM_PARITY_REFERENCE.md`** (the founder's actual 2-workflow Pipedream pipeline, reviewed line-by-line) — **this is now E3's spec.**
- [x] **H1 — curation-hardening BUILT, all 6 fixes, all §9 acceptance criteria PASS** (5 commits `b1585eb`→`b358fb6`, founder-approved 2026-07-09 "build approved... 11 assumptions"): entity cap enforced in curation, source-degradation banner (loud, snapshot-consistent), $0.50/day AI budget cap (verified `AI_ZERO_CALL` never falsely trips; real observed spend ≤$0.17/day), prompt-injection firewall, API-key URL scrubbing. Migrations `20260709000000`/`20260709000001` applied, advisors clean.
- [x] **I1 — insight-layer ARCHITECTURE.md committed** (`bbb35b0`), **founder-approved 2026-07-09 "build approved" — all 16 §7 assumptions as-is.** Three tiers: Tier A (causal "why," ships in E3, zero new AI cost), Tier B (deterministic decision-support stats — 52w position, valuation, computed server-side and *injected into* the AI grounding corpus, never AI-narrated — also ships in E3), Tier C (opt-in personal opinion lens, OFF by default, personalization firewall, dark for non-founder users pending attorney review — sequenced AFTER H1 [done] + E4, not yet built).
- [ ] **E3 — WF2 outputs + Tier A/B, dispatched 2026-07-09** (`docs/ops/PS_PROMPT_E3.md`). Founder correction baked in: market data renders **story-tied only**, never an unrelated watchlist dump. Spec = `PIPEDREAM_PARITY_REFERENCE.md`'s per-story output contract (source link, X+FB post pairs, video prompt+subtitles+overlay, chart, story-tied stats).
- [ ] **Tier C (I1) build** — tracked, not started. Sequenced after E4 (Analyze/Draft/Act wiring) per the architecture doc; needs its own dispatch when E4 lands.
- [ ] **E6 — host cutover + P7 backup + dogfood week** (founder-led; the V1 verdict). Pipedream subscription: keep until E6 passes (it's both the benchmark and the safety net) — see parity doc.
...
**Next founder actions:** judge the E3 brief vs Pipedream when it lands (the real gate this time — package + depth). **Next build after E3:** E4 (unblocks Tier C), then E5, then E6.
