...
> **S3 status (2026-06-15):** DONE pending founder review. Ticker/entity resolution (14 entities seeded, scorer extracts tickers, `resolveOrCreateTicker`, scored_articles.tickers/entity_ids + briefs.ticker_symbols populated) + market data (Finnhub quote/profile2/metric, CoinGecko crypto, Alpaca intraday — 11/11 tracked enriched w/ price/change/52w/PE, 9 w/ intraday) + NL market read (ticker_data.interpretation) woven into the brief. Migration `20260615000000` (advisors clean). Commits abb0648 + fd0a83c + c464a43. Proof: brief reads "TSLA $406.43 +1.82%… SPY/QQQ near 52-week highs".
> **S3.1 status (2026-06-15):** code DONE + verified; **Railway deploy pending (founder action)**. Confirmed root cause: dispatcher hits the Cloudflare host (ERR-026) → 3 score runs dangling `running`. Fixes (host-independent): batch score (N+1 killed; throws→`failed` not dangle), pg_cron stale-run sweep (heals dangling rows DB-side), curate consistency invariant (changed set → reset stale summary → self-heal). Migration `20260615010000`. Commits 3a208ce + 8ae95a9. The Railway move itself (deploy + secrets + `app_config` cutover) needs founder creds — runbook `docs/ops/railway-cutover.md`. **P7 off-platform backup now due.**
```
